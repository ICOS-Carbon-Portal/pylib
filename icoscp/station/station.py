...
def getIdList(project='ICOS', theme: list = None, sort='name', outfmt='pandas', icon=None):
    ...
    stn_df.sort_values(by=sort, inplace=True)

    if outfmt == 'map' and not stn_df.empty:
        stations_folium_map = fmap.get(stn_df, project, icon)
...
def getList(theme=None, ids=None):
    """
