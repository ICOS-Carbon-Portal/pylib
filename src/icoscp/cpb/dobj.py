...
from icoscp_core.auth import ConfigFileAuth, PasswordAuth, TokenAuth, \
    AuthToken, parse_auth_token
from icoscp_core import bootstrap
from icoscp_core import envri, auth, bootstrap
...
class Dobj():
    ...
    def __init__(self, digitalObject=None,
                 cp_auth: ConfigFileAuth | PasswordAuth | TokenAuth |
                          AuthToken | str = None):
    ...
    @token.setter
    def token(self, cp_auth: ConfigFileAuth | PasswordAuth | TokenAuth |
                          AuthToken | str | None = None):
        if isinstance(cp_auth, ConfigFileAuth | PasswordAuth | TokenAuth):
            self._token = cp_auth.get_token()
        elif isinstance(cp_auth, AuthToken):
            self._token = cp_auth
        elif isinstance(cp_auth, str):
            self._token = parse_auth_token(cp_auth)
        # If the Dobj class was not instantiated with an authentication
        # method, look for valid credentials in the  configuration file
        # in a system specific location.
        else:
            try:
                self._token = bootstrap.ConfigFileAuth(
                    conf=envri.ICOS_CONFIG).get_token()
            # Reaching this exception clause, means that there was no
            # way to authenticate the user before requesting data. When
            # `Dobj().data` is called, the class ConfigFileAuth() will
            # be instantiated and init_config_file() will be called.
    ...
    def lat(self):
        if self.valid:
            return float(self.meta['specificInfo']['acquisition']['station'][
                             'location']['lat'])
    ...
    def __getPayload(self):
        ...
        self._json = {
            ...
            'subFolder':
                self.meta['specification']['format']['uri'].split('/')[-1]
        }
    ...
    def __getColumns(self):
        ...
        # Access through HTTP request.
            ...
            # Since the authentication is not set, initialize a
            # configuration file, prompting for username and password.
            if not self.token:
                cp_auth = auth.ConfigFileAuth(conf=envri.ICOS_CONFIG)
                cp_auth.init_config_file()
                self.token = cp_auth

            request_headers = {'cookie': self.token.cookie_value}
    ...
    def __unpackRawData(self, rawData):
        ...
        def is_time(value_format):
            return value_format in time_formats

        def is_date(value_format):
            return value_format in date_formats

        def is_datetime(value_format):
            return value_format in datetime_formats

        def is_int(value_format):
            return value_format in integer_format
        ...
        if 'TIMESTAMP_END' in df.columns and self._dtconvert:
            df['TIMESTAMP_END'] = pd.to_datetime(df.loc[:, 'TIMESTAMP_END'],
                                                 unit='ms')
